//credits go to BI. Taken from ca.pbo from A1. Thus, APL license. (not APL-SA)
...
if ((count _this) == 7) then 
{
	...
	_adjX = _adjust select 0; 
	_adjY = _adjust select 1; 
	_adjZ = _adjust select 2;
};
...
if (_isObject) then 
{
	_light lightAttachObject [_object, [0 + _adjX, 0 + _adjY, -0.5 + _adjZ]];
} 
else 
{
	_lightPos = position _light; 
	if (_lightPos select 2 == 0) then 
	{
		_light setPos [_lightPos select 0, _lightPos select 1, 0.01];
	};
};
...
if (_isObject) then 
{
	_source2 setParticleParams [
		...
		0.2, 
		[0.2,(_intensity * 2)], 
		[[0,0,0,0], [0.3,0.3,0.3,1], [0.6,0.6,0.6,0.1], [0.6,0.6,0.6,0]], 
		[0,1,0,1,0,1], 
		0.2, 
		0.3, 
		"", 
...
if (_isObject) then 
{
	_source3 setParticleParams [
		...
		1, 
		(0.4 * (_intensity / 2)), 
		[0 + _adjX, 0 + _adjY, -1 + _adjZ], 
		[0, 0, 0], 
		1, 
		0.004, 
		0.004, 
		0.2, 
		[(0.1 + 0.05 * _intensity), (0.1 + 0.3 * _intensity), (0.1 + 0.02 * _intensity)], 
		[[1,1,1,0], [1,1,1,1], [1,1,1,0.6], [1,1,1,0]], 
		[0,1,0], 
		0.5, 
		0.05, 
		"", 
...
//do not delete vehicles etc! mode 1 == infinite I guess?
//used to be a while(true) loop!!
if (_mode == 1) exitWith {};
...
while {_cond} do {
	...
	if(_mode == 2) then {
		_cond = _isObject and (alive _object);
	}
	else {
		if(_mode == 3) then {
			_cond = _i < _maxI;
			...
			_cond = (alive _object) and (_i < _maxI) and _isObject;
		};
	};
};
